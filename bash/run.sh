...
python scripts/train_texture.py --config config/template.yaml --stamp $stamp --log_dir $log_dir --prompt "$prompt" --scene_id "$scene_id" --aov="$desired_aov" --checkpoint_step "$checkpoint_step"
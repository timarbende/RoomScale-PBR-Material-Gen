...
python scripts/train_texture_parametric.py --config config/template.yaml --stamp $stamp --log_dir $log_dir --prompt "$prompt" --scene_id "$scene_id" --aov="$desired_aov" --conditioning_texture_step "$conditioning_texture_step"